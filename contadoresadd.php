...
// Create page object
$contadores_add = new ccontadores_add();
$Page =& $contadores_add;

// Page init
$contadores_add->Page_Init();

// Page main
$contadores_add->Page_Main();
?>
...
// Create page object
var contadores_add = new ew_Page("contadores_add");

// page properties
contadores_add.PageID = "add"; // page ID
contadores_add.FormID = "fcontadoresadd"; // form ID
var EW_PAGE_ID = contadores_add.PageID; // for backward compatibility

// extend page with ValidateForm function
contadores_add.ValidateForm = function(fobj) {
...
contadores_add.Form_CustomValidate =  
...
contadores_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
contadores_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $contadores->TableCaption() ?><br><br>
<a href="<?php echo $contadores->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$contadores_add->ShowMessage();
?>
<form name="fcontadoresadd" id="fcontadoresadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return contadores_add.ValidateForm(this);">
<p>
...
class ccontadores_add {
	...
	// Page object name
	var $PageObjName = 'contadores_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $contadores->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$contadores->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $contadores->CurrentAction = "C"; // Copy record
		  } else {
		    $contadores->CurrentAction = "I"; // Display blank record
		...
		switch ($contadores->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$contadores->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $contadores->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "contadoresview.php")
						$sReturnUrl = $contadores->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$contadores->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function AddRow() {
		...
		$bInsertRow = $contadores->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($contadores->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$contadores->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $contadores->id->DbValue;

			// Call Row Inserted event
			$contadores->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
