...
// Create page object
var ircio_add = new ew_Page("ircio_add");

// page properties
ircio_add.PageID = "add"; // page ID
ircio_add.FormID = "fircioadd"; // form ID
var EW_PAGE_ID = ircio_add.PageID; // for backward compatibility

// extend page with ValidateForm function
ircio_add.ValidateForm = function(fobj) {
...
ircio_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
ircio_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $ircio->TableCaption() ?><br><br>
<a href="<?php echo $ircio->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$ircio_add->ShowMessage();
?>
<form name="fircioadd" id="fircioadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ircio_add.ValidateForm(this);">
<p>
...
class circio_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $ircio->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$ircio->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $ircio->CurrentAction = "C"; // Copy record
		  } else {
		    $ircio->CurrentAction = "I"; // Display blank record
		...
		switch ($ircio->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$ircio->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $ircio->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "ircioview.php")
						$sReturnUrl = $ircio->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$ircio->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		if ($ircio->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($ircio->RowType == EW_ROWTYPE_ADD) { // Add row
			...
			$ircio->orden->EditValue = ew_HtmlEncode($ircio->orden->CurrentValue);

			// op
			$ircio->op->EditCustomAttributes = "";
			$ircio->op->EditValue = ew_HtmlEncode($ircio->op->CurrentValue);
	...
	function AddRow() {
		...
		if ($ircio->orden->CurrentValue == "" && $ircio->orden->getSessionValue() == "") {
			$this->setMessage($Language->Phrase("InvalidKeyValue"));
			return FALSE;
		}

		// Check if key value entered
		if ($ircio->op->CurrentValue == "" && $ircio->op->getSessionValue() == "") {
			$this->setMessage($Language->Phrase("InvalidKeyValue"));
			return FALSE;
		}
		...
		// orden
		$ircio->orden->SetDbValueDef($rsnew, $ircio->orden->CurrentValue, "", FALSE);

		// op
		$ircio->op->SetDbValueDef($rsnew, $ircio->op->CurrentValue, "", FALSE);
		...
		$bInsertRow = $ircio->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($ircio->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
